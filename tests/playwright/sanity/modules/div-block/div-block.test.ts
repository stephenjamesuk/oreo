...
test.describe( 'Div Block tests @div-block', () => {
	...
	test( 'Dragging an element from the panel to a flexbox with div block should render in the correct position', async ( { page, apiRequests }, testInfo ) => {
		const wpAdmin = new WpAdminPage( page, testInfo, apiRequests );
		const editor = await wpAdmin.openNewPage();

		const flexbox = await editor.addElement( { elType: 'e-flexbox' }, 'document' );
		await editor.addElement( { elType: 'e-div-block' }, flexbox );

		// Open widget panel.
		await page.getByLabel('Add Element').click();

		const sourceElement = page.locator(
			'.elementor-element:has(.title:has-text("Heading")):has(i.eicon-atomic)'
		);

		const targetElement = editor.getPreviewFrame().locator( getElementSelector( flexbox ) );
